...
STAGING="$PACKAGE_ROOT/service-admin-$PLATFORM"
TAR_PATH="$DIST/service-admin-$PLATFORM.tar.gz"
...
cp "$ROOT/runtime/$PLATFORM/start-service-admin.sh" "$STAGING/runtime/start-service-admin.sh"
...
chmod +x "$STAGING/runtime/start-service-admin.sh" 2>/dev/null || true
