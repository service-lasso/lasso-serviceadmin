...
type ServiceRouteRow = {
  ...
  bind: string
  port: number
  protocol: NonNullable<ServiceEndpoint['protocol']>
  exposure: NonNullable<ServiceEndpoint['exposure']>
...
function buildServiceRouteRows(services: DashboardService[]) {
  return services.flatMap((service) =>
    service.endpoints.map((endpoint) => {
      ...
      const bind = endpoint.bind ?? 'unknown'
      const port = endpoint.port ?? 0
      const protocol = endpoint.protocol ?? 'unknown'
      const exposure = endpoint.exposure ?? 'local'
      const provider = providerForExposure(exposure)
      const parsedRoute = parseRouteUrl(url)
      const routeState = routeStateForEndpoint(
        service.status,
        parsedRoute.hasValidUrl
      )
      const row: ServiceRouteRow = {
        id: `${service.id}:${endpoint.label}:${url}`,
