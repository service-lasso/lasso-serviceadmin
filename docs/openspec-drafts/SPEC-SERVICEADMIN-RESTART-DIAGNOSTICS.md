...
Distinguish service-detail request readiness from trusted-identity unlock before
the packaged Broker lifecycle test clicks Restart. This is test-only observation,
not a product UI change or proof of the historical macOS failure's cause.
...
- `RD-004`: Cover pending, failed response, missing service, ready response,
  ...
  missing controls fail with closed metadata and present controls proceed.
