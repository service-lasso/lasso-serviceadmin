...
- Security > Rotations keeps the mixed dry-run fixture non-mutating. Live migrate_remap_provider apply uses the broker campaign contract: dry-run, revalidate immediately before apply, audit reason, and typed exact campaign id. Other apply families fail closed as unsupported and never report metadata-only success. Partial success stays visible; retry only retry-safe operation IDs.
