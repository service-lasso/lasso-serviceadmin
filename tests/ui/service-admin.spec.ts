...
test('dashboard renders and starts stopped services', async ({ page }) => {
  ...
  await expect(page.getByText('Listen ports', { exact: true })).toBeVisible()
  await expect(page.getByText('Inbox unread', { exact: true })).toBeVisible()
  await expect(page.getByText('Generation lane', { exact: true })).toBeVisible()
  await expect(page.getByText('Log volume', { exact: true })).toBeVisible()
  await expect(page.getByText('Network exposure')).toHaveCount(0)
